#ifndef EDGE_PATTERN_DATABASE_1_H
#define EDGE_PATTERN_DATABASE_1_H
...
public:
    ...
    {
        int targetEdges[6] = {0, 1, 2, 3, 4, 5};
        int currentPositions[6];
        int orientations[6];

        // 1. Bitboard Extraction using your getEdge() method
        for (int j = 0; j < 12; j++)
        {
            CubeBitB::EdgePiece edge = cube.getEdge(j);

            // We only care about edges with IDs 0, 1, 2, 3, 4, 5
            if (edge.id < 6)
            {
                currentPositions[edge.id] = j;
                orientations[edge.id] = edge.orientation;
            }
        }
