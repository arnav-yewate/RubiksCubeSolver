...
public:
    ...
    uint32_t getDatabaseIndex(const CubeBitB &cube) const
    {
        ...
        for (int j = 0; j < 12; j++)
        {
            ...
            // We only care about edges with IDs 6 through 11
            if (edge.id >= 6 && edge.id <= 11)
            {
                // Map edge ID 6 to index 0, ID 7 to index 1, etc.
                currentPositions[edge.id - 6] = j;
                orientations[edge.id - 6] = edge.orientation;
            }
        }
